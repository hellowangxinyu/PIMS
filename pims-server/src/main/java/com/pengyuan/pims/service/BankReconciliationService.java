    ...
    public Map<String, Object> autoMatch(Long accountId) {
        return writeQueue.executeTx(() -> {
            ...
            List<Map<String, Object>> receipts = jdbc.queryForList(
                    "SELECT id AS jid, doc_no AS docNo, strftime('%Y-%m-%d', receipt_date/1000, 'unixepoch', 'localtime') AS d, amount, customer_name AS party FROM payment_receipt WHERE bank_account = ? ORDER BY receipt_date", accountName(accountId));
            List<Map<String, Object>> disbs = jdbc.queryForList(
                    "SELECT id AS jid, doc_no AS docNo, strftime('%Y-%m-%d', pay_date/1000, 'unixepoch', 'localtime') AS d, amount, supplier_name AS party FROM payment_disbursement WHERE bank_account = ? ORDER BY pay_date", accountName(accountId));
            int auto = 0;
