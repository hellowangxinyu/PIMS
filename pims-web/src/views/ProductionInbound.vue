<template>
  <div class="page-container">
    ...
    <div class="table-card">
      ...
      <p-table :data="rows" stripe border style="width:100%" @selection-change="sel => selectedRows = sel">
        ...
        <el-table-column label="实际材料成本" width="130" align="right">
          <template #default="{ row }">
            <span v-if="actualCostMap[row.productionOrderNo] != null" class="cost-cell" title="按领料出库批号单价汇总">￥{{ Number(actualCostMap[row.productionOrderNo]).toFixed(2) }}</span>
            <span v-else class="text-muted">-</span>
          </template>
        </el-table-column>
        <el-table-column label="单位成本" width="110" align="right">
          <template #default="{ row }">
            <span v-if="actualCostMap[row.productionOrderNo] != null && Number(row.qty) > 0" class="cost-cell">￥{{ (Number(actualCostMap[row.productionOrderNo]) / Number(row.qty)).toFixed(2) }}/{{ row.unit || 'kg' }}</span>
            <span v-else class="text-muted">-</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="状态" width="90" align="center">
          <template #default="{ row }">
            <el-tag :type="prodStatusType(row.status)" size="small">{{ prodStatusLabel(row.status) }}</el-tag>
          </template>
        </el-table-column>
    ...
    <!-- 参照生产订单入库 -->
    <el-dialog title="参照生产订单入库" v-model="visible" width="min(1100px, 96vw)" destroy-on-close>
      <el-form :model="form" label-width="100px">
        <el-form-item label="生产订单" required>
          <el-select v-model="form.productionOrderId" filterable placeholder="选择已确认的生产订单" style="width:100%" @change="onOrderChange">
    ...
    <el-dialog title="配方溯源" v-model="traceVisible" width="min(1100px, 96vw)" destroy-on-close>
      ...
      <div v-else class="trace-content">
        ...
        <div v-if="traceData.outboundBatches && traceData.outboundBatches.length" class="batch-section">
          <h4 style="margin:16px 0 8px;font-size:14px">实际领料批次追溯</h4>
...
async function doExport() {
  ...
  try {
    await downloadFile('/outbound/production-inbound/export', { keyword: searchText.value || undefined }, `生产入库-${new Date().toLocaleDateString('sv')}.xlsx`)
...
const form = ref({ productionOrderId: null, qty: null, batchNo: '', warehouseId: '', zoneId: '', locationId: '', remark: '' })

// 实际材料成本：按生产订单汇总的领料出库批号成本
const actualCostMap = ref({})
async function fetchActualCosts() {
  try {
    const costs = await api.get('/outbound/production/actual-costs')
    const map = {}
...
const selectedOrder = computed(() => confirmedOrders.value.find(o => o.id === form.value.productionOrderId))
...
async function fetch() {
  ...
  try {
    const res = await api.get('/outbound/production-inbound', { params })
    rows.value = res.rows
...
async function openDialog() {
  form.value = { productionOrderId: null, qty: null, batchNo: '', warehouseId: '', zoneId: '', locationId: '', remark: '' }
  zones.value = []
  locations.value = []
  try { confirmedOrders.value = await api.get('/outbound/referenceable-production-orders', { params: { forType: 'inbound' } }) } catch {}
  visible.value = true
}
...
async function submit() {
  ...
  loading.value = true
  try {
    await api.post('/outbound/production-inbound', null, {
      params: {
...
async function openTrace(row) {
  ...
  try {
    // 通过生产订单号找到订单ID
    const orders = await api.get('/production-order')
    const order = orders.find(o => o.orderNo === row.productionOrderNo)
    if (!order) { traceData.value = { hasRecipe: false, orderNo: row.productionOrderNo }; return }
    const data = await api.get(`/production-order/${order.id}/trace`)
