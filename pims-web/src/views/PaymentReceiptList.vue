...
const form = ref({ customerId: null, amount: null, method: 'BANK', bankAccount: '', receiptDate: new Date().toISOString().slice(0,10), remark: '' })
...
async function doExport() {
  ...
  try {
    await downloadFile('/finance/receipt/export', {}, `收款单-${new Date().toISOString().slice(0, 10)}.xlsx`)
...
function openDialog() {
  form.value = { customerId: null, amount: null, method: 'BANK', bankAccount: '', receiptDate: new Date().toISOString().slice(0,10), remark: '' }
  fetchCustomer()
  fetchAR()
  visible.value = true
}
