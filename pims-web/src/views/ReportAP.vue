  ...
  <div class="report-page">
    ...
    <div class="table-card">
      <div class="tab-toolbar">
        <span class="tab-count">共 {{ apList.length }} 条</span>
      </div>
      <p-table :data="pagedRows" stripe border style="width:100%">
        ...
        <el-table-column prop="purchaseOrderNo" label="采购订单" min-width="120" show-overflow-tooltip />
        <el-table-column prop="arrivalInfo" label="到货单" min-width="160" show-overflow-tooltip>
          <template #default="{ row }">{{ row.arrivalInfo || '—' }}</template>
        </el-table-column>
        <el-table-column label="类型" width="90" align="center">
          <template #default="{ row }">{{ payableTypeLabel(row.payableType) }}</template>
        </el-table-column>
        <el-table-column prop="amount" label="应付金额" width="120" align="right" v-if="hasAmountPerm('finance-ar')">
          <template #default="{ row }">¥{{ fmt(row.amount) }}</template>
        </el-table-column>
        <el-table-column prop="paidAmount" label="已付金额" width="120" align="right" v-if="hasAmountPerm('finance-ar')">
        ...
        <el-table-column label="剩余" width="120" align="right" v-if="hasAmountPerm('finance-ar')">
          <template #default="{ row }">¥{{ fmt((row.amount||0) - (row.paidAmount||0)) }}</template>
        </el-table-column>
        <el-table-column label="状态" width="100" align="center">
          <template #default="{ row }">
            <el-tag :type="apStatusType(row.status)" size="small">{{ apStatusLabel(row.status) }}</el-tag>
          </template>
        </el-table-column>
...
async function doExport() {
  ...
  try {
    await downloadFile('/finance/ap/export', {}, `应付明细-${new Date().toISOString().slice(0, 10)}.xlsx`)
  } catch (e) { /* downloadFile 内已提示 */ }
  finally { exporting.value = false }
}

// 状态中文映射
const apStatusType = (s) => globalStatusType(s, {UNPAID: 'danger', PARTIAL: 'warning', PAID: 'success'})   // v6.6 收口：全局 + 域局部
function apStatusLabel(s) { return { UNPAID: '未付款', PARTIAL: '部分付款', PAID: '已结清' }[s] || '未知' }
function payableTypeLabel(t) { return { PURCHASE: '采购', OUTSOURCE: '委外加工', OTHER: '其他' }[t] || '未知' }

async function loadAP() {
  resetPage()
  try { apList.value = await api.get('/finance/ap') } catch {} }


// ===== 分页（v5.2）：超过 50 条自动分页展示 =====
const { page, pageSize, pagedRows, resetPage } = usePaging(apList)
