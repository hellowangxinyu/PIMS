<template>
  <div class="page-container">
    <div class="page-header">
      <h2>付款单</h2>
      <el-button type="primary" @click="openDialog" v-if="hasPerm('finance:write')">新建付款单</el-button>
    ...
    <div class="table-card">
      ...
      <p-table :data="pagedRows" stripe border style="width:100%">
        ...
        <el-table-column prop="apDocNo" label="关联应付单" min-width="160" show-overflow-tooltip>
          <template #default="{ row }">
            <span v-if="row.apDocNo">{{ row.apDocNo }}</span>
            <span v-else style="color:#94a3b8">—</span>
          </template>
        </el-table-column>
        <el-table-column prop="supplierName" label="供应商" min-width="180" show-overflow-tooltip />
        <el-table-column prop="amount" label="付款金额" width="130" align="right" v-if="hasAmountPerm('finance-ar')">
        ...
        <el-table-column label="操作" width="110" align="center" v-if="hasPerm('finance:write')">
          <template #default="{ row }">
            <button class="op-btn op-btn-primary" :disabled="genDlg?.hasGenerated('PAYMENT', row.docNo)"
              @click="genDlg?.open('PAYMENT', row.id)">
              {{ genDlg?.hasGenerated('PAYMENT', row.docNo) ? '已生成' : '生成凭证' }}
            </button>
          </template>
        </el-table-column>
      </p-table>
    ...
    <!-- 新建付款单弹窗 -->
    <el-dialog title="新建付款单" v-model="visible" width="min(1100px, 96vw)" destroy-on-close>
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="未结清应付" v-if="form.supplierId">
          <div class="unpaid-tip">
            <span>共 {{ unpaidCount }} 张未结清应付单</span>
...
const apList = ref([])  // 全部应付单（用于按供应商计算未结清金额）
...
// v5.23：导出全部付款单流水（金额列按后端 finance:amount 权限脱敏）
...
const currentUnpaidList = computed(() => {
  ...
  return apList.value.filter(a => a.supplierId === form.value.supplierId && a.status !== 'PAID')
})
const unpaidCount = computed(() => currentUnpaidList.value.length)
const unpaidTotal = computed(() => currentUnpaidList.value.reduce((s, a) => s + (Number(a.amount || 0) - Number(a.paidAmount || 0)), 0))

async function fetch() {
  resetPage()
  try { list.value = await api.get('/finance/disbursement') } catch {} }
async function fetchSupplier() { try { supplierList.value = await api.get('/supplier', { params: { enabled: true } }) } catch {} }
...
async function submit() {
  ...
  if (!form.value.amount || form.value.amount <= 0) { ElMessage.warning('付款金额必须大于0'); return }
  loading.value = true
  try {
    await api.post('/finance/disbursement', form.value)
